          ...
          <button onclick="location.href = '<?php echo site_url('pemesanan1/view_data_penjualan_detail2/'.$cacah['id_pesanan']) ?>'" type="button" class="btn btn-info btn-sm">
					...
					<td>
						<?php if ($cacah['status']=='selesai') {
							...
							?>
							<button onclick="location.href = '<?php echo site_url('pemesanan1/edit_form/'.$cacah['id_pesanan'].'/'.$cacah['id_konsumen']) ?>'" type="button" class="btn btn-success btn-sm">
						<span class="glyphicon glyphicon-menu-hamburger"></span> Tambah
						<?php
						} ?>
					</td>		
				</tr>
